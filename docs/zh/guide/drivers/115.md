...
3. 将获取到的 <Badge text="Token" type="info" vertical="middle" /> 添加到 AList 115 驱动的 **`二维码令牌`** 选项内，然后选择 `Qrcode源` 设备（推荐选择 `tv`（TV），Cookie 选项留空）
   - 可选设备：`Web`，`android`，`ios`，`tv`，`alipaymini`，`wechatmini`，`qandroid`
     - 推荐选择 `tv`（TV）
     - 不推荐选择 `Web`、`android` 或 `ios`，因为使用常用设备登录后可能会将原有登录状态挤下线
...
- 大目录现在会按页获取，目录中文件很多时，列目录会更稳定。
- 当 115 上游提供缩略图时，AList 现在会把缩略图地址透出给前端，方便图片和视频卡片使用平台缩略图。
- 下载请求会在需要时保留调用方的 `User-Agent`，从而改善部分 115 客户端和播放场景的兼容性。
