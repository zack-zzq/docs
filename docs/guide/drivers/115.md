...
- Thumbnail exposure, paginated listing improvements, and better download compatibility: `>= v3.58.0`
:::
...
3. Add the obtained <Badge text="Token" type="info" vertical="middle" /> to the **`QRcode Token`** option of the AList 115 Cloud driver, then select a `Qrcode source` device (`tv` (TV) is recommended; leave the Cookie option blank)
   - Equipment to choose from：`Web`、`android`、`ios`、`tv`、`alipaymini`、`wechatmini`、`qandroid`
     - `tv` (TV) is recommended.
     - `Web`, `android`, and `ios` are not recommended because logging in with a commonly used device may sign out the existing session.
...
- Large folders are now fetched page by page, which is more reliable when a directory contains many items.
- AList now exposes 115 thumbnail URLs when the upstream provides them, so image and video cards can use platform thumbnails.
- Download requests preserve the caller's `User-Agent` when needed, which improves compatibility with some 115 clients and playback scenarios.
