...
- Select **"Terminal (Admin)"** or **"Windows PowerShell (Administrator)"**

### 2. Run Installation Script
- Paste the script command in the administrator terminal
- Press Enter to execute the script

### 3. Interactive Installation Interface
After the script starts, an interactive installation interface will appear with the following options:
- **Install/Update Alist**
- **Uninstall Alist**
...
**Installation Notes:**
- Default installation path: `C:\alist`
- When registering system service, the script will automatically download NSSM program
- Add Alist service to Windows system services, after successful registration, Alist will start automatically with Windows boot

### 4. Service Startup & Configuration
**After service starts automatically, you will see:**
- Default username and password generated by Alist
- Web login address

**Password Modification Methods:**
- **Method 1:** Through Web interface → Profile → Change Password
- **Method 2:** Command line modification
  ```cmd
  cd C:\alist
...
- Select option **6** in the interactive interface to start service
...
1. First select option **5** to delete the registered Windows system service
2. Then select option **3** to uninstall Alist program
3. After uninstallation, all files in the installation path will be deleted

> **Note:** Please ensure important data is backed up before uninstalling
